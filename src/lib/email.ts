...
export async function sendPasswordResetEmail(email: string, token: string, name: string) {
  ...
  try {
    await transporter.sendMail({
      from: process.env.EMAIL_FROM || 'Sanwaria Fashion <noreply@sanwaria.com>',
      to: email,
      subject: 'Reset Your Sanwaria Fashion Password',
      html: `
        <div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 600px; margin: 0 auto; background: #fff;">
          <div style="background: linear-gradient(135deg, #1a1a2e 0%, #16213e 100%); padding: 40px; text-align: center;">
            <h1 style="color: #d4af37; font-size: 28px; margin: 0; letter-spacing: 2px;">Sanwaria FASHION</h1>
          </div>
          ...
          <div style="background: #1a1a2e; padding: 20px; text-align: center;">
            <p style="color: #666; font-size: 12px; margin: 0;">© 2024 Sanwaria Fashion. All rights reserved.</p>
          </div>
        </div>
      `,
    });
...
export async function sendOrderConfirmationEmail(email: string, name: string, orderId: string, total: number) {
  await transporter.sendMail({
    from: process.env.EMAIL_FROM || 'Sanwaria Fashion <noreply@sanwaria.com>',
    to: email,
    subject: `Order Confirmed #${orderId.slice(-8).toUpperCase()} — Sanwaria Fashion`,
    html: `
